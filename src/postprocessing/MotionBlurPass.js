...
import { createCompositeMaterial } from "./CompositeShader.js";
...
const _blackColor = new Color(255, 255, 0);
...
export class MotionBlurPass extends Pass {
    ...
    constructor(scene, camera, options = {}) {
        ...
        this.smearIntensity =
            "smearIntensity" in options ? options.smearIntensity : 5;
        ...
        this._compositeMaterial = createCompositeMaterial();
        this._compositeQuad = new FullScreenQuad(this._compositeMaterial);
        // console.log("Composite Material:", this._compositeMaterial);
        // console.log("FullScreenQuad Material:", this._compositeQuad.material);
    }
    ...
    setSize(width, height) {
        ...
        if (width > 0 && height > 0) {
            this._velocityBuffer.setSize(
                width * renderTargetScale,
                height * renderTargetScale
            );
        } else {
            console.error(
                "Invalid dimensions for render target:",
                width,
                height
            );
        }
    ...
    render(renderer, writeBuffer, readBuffer) {
        ...
        switch (debug.display) {
            ...
            case MotionBlurPass.DEFAULT: {
                // console.log("Velocity Buffer:", this._velocityBuffer);
                const velocityBuffer = this._velocityBuffer;
                const textureType = renderer.capabilities.isWebGL2
                    ? HalfFloatType
                    : FloatType;
                if (this._velocityBuffer && this._velocityBuffer.texture) {
                    if (!this._velocityBuffer.texture.image) {
                        console.error(
                            "Velocity buffer texture image is undefined."
                        );
                        return;
                    }
                    ...
                    if (
                        ...
                        !renderer.capabilities.floatTextures
                    ) {
                        console.error(
                            "HalfFloatType or FloatType textures are not supported."
                        );
                    }
                    try {
                    ...
                    console.error(
                        "Velocity buffer is not initialized properly."
                    );
                ...
                if (
                    compositeMaterial.defines.JITTER_STRATEGY !==
                    this.jitterStrategy
                ) {
                    compositeMaterial.defines.JITTER_STRATEGY =
                        this.jitterStrategy;
    ...
    _drawMesh(renderer, mesh, type, saveState) {
        ...
        const isTransparent =
            mesh.material.transparent || mesh.material.alpha < 1;
        const isCulled =
